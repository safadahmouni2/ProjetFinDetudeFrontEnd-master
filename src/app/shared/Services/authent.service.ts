...
export class AuthentService {
  ...
  constructor(private httpClient: HttpClient) { }
    ...
    register(name_user,first_name,email
             ,title,login,pwd,confirmPassword,gender,age,phone,date_birth,pays,description, file: File)  {
        const data: FormData = new FormData();
        data.append('file', file, file.name);
        data.append('u', new Blob([JSON.stringify({
            name_user,first_name,email,title,login,pwd,confirmPassword,gender,age,phone,date_birth,pays,description
    ...
    convertToken(){
       const token = localStorage.getItem('AuthToken');
        ...
        const userData = { id: decodedToken.id,
